...
class TencentMovieListUpdateSpider(scrapy.Spider):
    name = "tencent_movie_list_update"
    allowed_domains = ["qq.com"]
    start_urls = ['http://qq.com/']

    def start_requests(self):
        self.connect_mysql()
        self.base_url = "https://v.qq.com/x/bu/pagesheet/list?_all=1&append=0&channel=movie&sort=19&listpage=2&pagesize=30"
        self.key = ""
        self.key_val = ""
        # 从数据库获取所有的分类
        self.cursor.execute("select * from movie_category where disabled = 0")
        ...
        for category in data:
            if category['key1'] == 'sort' or category['key_val'] == '-1':
                ...
                if key == 'iyear':
                   ...
                   url = self.base_url + "&" + 'year' + "=" + key_val + "&offset=0"
